...
(function ($) {
    ...
    $.fn.countdown = function (opt) {
        var time = opt.time ? opt.time : {h:-1,m:-1,s:-1};
        if (time.d < 0 || time.h < 0 || time.m < 0 || time.s < 0) return;
        if (time.h > 23 || time.m > 59 || time.s > 59) return;
        var theCountdown = $.countdown(this, opt);
        theCountdown.tick();
        setInterval(theCountdown.tick, 1000);
    };
}(jQuery));
